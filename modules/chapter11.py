...
def _render_results(section_key: str, extra_note: str = "") -> None:
    ...
    if improved == "__AT_CEILING__":
        st.markdown("---")
        ceiling_msg = data.get("ceiling_message", "Add specific numbers, unique achievements, or personal details that only you know.")
        max_score = SECTION_MAX[section_key]
        st.info(
            f"🎯 You're at {score}/{max_score} — near perfect!\n\n"
            f"To reach {max_score}/{max_score}, personalize with:\n"
            "• Add a specific number or metric only you know\n"
            "• Add a unique achievement or recognition\n"
            "• Add a personal detail that makes it yours\n\n"
            f"{ceiling_msg}\n\n"
            "Edit the version above directly, then re-evaluate to get your final score."
        )
    elif improved:
        st.markdown("---")
